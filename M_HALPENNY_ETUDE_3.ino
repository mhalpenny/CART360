...
void live()
{
   ...
   if (analogRead(NOTE_IN_PIN) > 8){ //only play if its past a noise threshhold
   int frequency = ((analogRead(NOTE_IN_PIN) + ((analogRead(PHOTO_PIN))*4))/8); //Modulate NOTE_IN by PHOTO values 
...
void updateTimer()
{
timePassed = millis() - startTime; // how much time has passed
activeFrequency = testNote; // store note value to call later in array
...
void playCurrentNote()
{
  offsetFrequency = getPhotoFrequency();
    ...
    if(analogRead(NOTE_IN_PIN) > 8){ //play note if threshold passed

      tone(BUZZER_PIN, (analogRead(NOTE_IN_PIN) + (getRunningAverage()*4)), duration);  //note played is a combination of NOTE_IN & PHOTO_CELL
    }
...
notes[countNotes] = activeFrequency; //updates array values
durations[countNotes] = timePassed;

countNotes++; //counts array number
timePassed=0; //resets timer
}
...
{
  analogRead(PHOTO_PIN); // returns value to variable
...
{
  int sensorVal = getPhotoFrequency();
  int sampleTotal = 0;
  
  
  runningAverageBuffer[nextCount] = sensorVal; // stores sensor values in array to be used below
  nextCount++;
  
  if(nextCount >= RUNNING_SAMPLES){
    nextCount = 0; //resets count for average value buffer
  }
  ...
  for(int count = 0; count < RUNNING_SAMPLES; count++){
    ...
    currentSum += runningAverageBuffer[count]; //sums all values stored over array
  }

  int averageVal = (currentSum / RUNNING_SAMPLES); //averages values
...
{
for (int count=0; count < countNotes; count++){
tone(BUZZER_PIN, notes[count], durations[count]); //plays through note array
delay(durations[count]); // ensures arduino doesnt play next note before current finishes


 if(digitalRead(BUTTON_MODE_PIN) == HIGH) //allow mode switching
      break;
      
delay(100); //delay between notes
colorLED((notes[count])/3); // change LED depending on note 
