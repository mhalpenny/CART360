...
void sendToShiftRegister(int potData)
{
  digitalWrite(LATCH_PIN, LOW); //prepares shift registry for data
  shiftOut(DATA_PIN, CLOCK_PIN,  MSBFIRST, hexArray[potData]);        //shifts array data out according to potentiometer value
  digitalWrite(LATCH_PIN, HIGH);  //opens register for data in
}
...
int getAnalog() {
  return (analogRead(POT_PIN) / 64);   // map potentiometer to array values (0-15)
}
