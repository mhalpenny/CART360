...
**************************************************************************/
void reset() {
  countNotes = 0; //reset step counter
  noTone(BUZZER_PIN); //stop playback

  digitalWrite(LATCH_PIN, LOW); //turn off seven segment display
  storeOnce = ISFALSE;
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES) {               // confirm that 16 notes have not been recorded yet
    ...
    if (testNote > theAdjuster && (t1.getTimePassed() < MAX_PRESS_TIME)) {
      ...
      */
      if (activeNoteButton == ISFALSE) {
        ...
        if (countNotes) {               // ensures no note is being played
          addToArrayInBetween();
        }
      ...
      */
        ...
        t1.updateTimer();             // update the timer (for durations)
    ...
    */
      ...
      t_between.startUpTimer();           //starts recording time intervals between notes using second timer
      updateArraysWithNoteAndTimings();   //saves timing into array
    }
  }
  if (storePatternInterruptRequest == ISTRUE) {  // make sure that in record mode we cannot save a pattern
    storePatternInterruptRequest = ISFALSE;
...
void addToArrayInBetween() {
  ...
  t_between.updateTimer(); //starts pause interval timer
  
  durations_In_Between[countNotes] = t_between.getTimePassed();     // saves the interval time

  t_between.setTimePassed(0);     // resets timer
}
...
void selectThePatternWithPot_Stage_0_(int & shiftRef)
{
  while (storePatternInterruptRequest == ISFALSE && modeChangeInterruptRequest == ISFALSE) {
    ...
    shiftRef = getAnalog(); //returns scaled potentiometer value from ShiftRegister.h
    //Serial.println(shiftRef);

    if (isValidIndex(shiftRef) == ISFALSE) {  // if index is not valid
      blinkSevenSegmentDisplay(); //see ShiftRegister.h
    }

    else {                                    // index is valid
      sendToShiftRegister(shiftRef);  //index is valid to pass potentiometer value to shift register
    }
  }
}
...
{
  while (modeChangeInterruptRequest == ISFALSE) {       //while mode button is LOW
    if (isValidIndex(shiftRef) == ISTRUE) { //if patterns exist

      const int * noteArray = retrievePatternNotes(shiftRef); //retrieve pattern selection (potentiometer value)
      
      const int * durationsArray = retrievePatternDurations(shiftRef); //retrieve durations
      const int * pausesArray =  retrievePatternPauses(shiftRef); //retrieve pauses

      int i = 0;
      countNotes = getNumberNotesOfPattern(shiftRef); // loads amount of notes from index

      while (storePatternInterruptRequest == ISFALSE && modeChangeInterruptRequest == ISFALSE ) { //if not disrupted by input
        if (countNotes > 0) {     // test values
          tone(BUZZER_PIN, noteArray[i], durationsArray[i]);      //plays back pattern
          delay(durations[i]);
          delay(pausesArray[i]);

          if (i < countNotes - 1) { //cycle through only recorded notes
            i++;
          } else {
            i = 0; //reset counter and restart pattern
            delay(DURATION);
          }
        }
      }
...
void retreiveMode()
{
  ...
  while (patternListIsEmpty() == ISTRUE && modeChangeInterruptRequest == ISFALSE) {   //if there are no patterns stored and mode has not been pressed
...
/**************************************************************************
  ...
  1. The reason to use a shift register is to extend the amount of digital pins the Arduino can manipulate.
  Since the Uno has a limited amount of pins when using mulitple sensors/input devices a shift register helps extend the 
  microcontrollers abilities. In this assignment it is used to add enough pins to control a seven segment display,
  which needs 8 pins. The registry functions by taking in serial values from the Arduino instead of a linear comm format. 
  This is done by using coded signals within the data line to send a "start" and "stop" signal to tell the chip when to read
  data. This way we only need to send one data cable to the shift register as all the information can travel in this serial channel.
 
  2. To allow the user to switch between patterns while in retrieve mode (as they can't now) one would need to program
  another interupt function. The easiest way to do this would be to select the potentiometer as the interuptting value. When
  selected pattern is playing the following code (while (storePatternInterruptRequest == ISFALSE && modeChangeInterruptRequest == ISFALSE )) 
  can be supplemented to include the potentiometer interupt function, which would resend the user to retrieveMode.

  3. If one were to add in modulation via the photocell and record durations one would have to implement a timer for that 
  duration that doubled as a method to store the modulations in a large array (x amount of change per y amount of time over the duration)
  This array would have to utilize the original notes and take pauses into consideration or the pauses could be modulated.
  More specifically a basic function would read the note array, create an equal length timer to that of the duration of said note,
  and store modulated note values over that time by a prefered amount of ms (the longer, the less data). Instead of then calling notes
  and duration the modulated note could be called instead along with the pauses. 
  
  4. IDEA 1: As someone who frequently uses step sequences such as bass machines and drum machines the first intuitively missing 
  component is tempo control. Technically one could press the buttons at the speed they wanted but once a pattern is recorded 
  we don't have any room to modulate the pattern post-recording. Tempo control would be a great, easy extension for this sequencer.
  A basic way to implement this would be to take another potentiometer and allow the user to scale duration values and their 
  corresponding delays.

  IDEA 2: Another idea that has partially been implemented but not fully explored would be to have several pattern playbacks. 
  Right now the patterns are played in order of recording, and in Etude 3 we explored a random playback function. But the 
  difference here would be similar to analogue sequencers that have basic settings such as "up" and "down", so forward or backwards
  and additional setting such as gates, shift, etc. Adding this little features would again allow more post-recording sound 
  customization, meaning once you record a fun pattern you can still play around with it without having to record a new one. 
